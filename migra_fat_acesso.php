...
foreach ($result as $row)
{
    ...
    $id_ip = $row['ip'];
    $duracao = $row['duracao_navegacao'];
	$carregamento = $row['duracao_carregamento'];
	$tamanho = $row['tamanho_trafegado'];
    
    $sql = "INSERT INTO FAT_ACESSO 
                        (id_pagina, id_pagina_entrada, id_pagina_saida, id_endereco, id_cliente, id_resolucao, id_dispositivo, id_navegador, id_ip, id_data, id_tempo, duracao_navegacao, duracao_carregamento, tamanho_trafegado)
                    VALUES (
